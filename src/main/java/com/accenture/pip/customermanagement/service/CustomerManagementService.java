...
import com.accenture.pip.customermanagement.exception.UserNameAlreadyInUseException;
...
public class CustomerManagementService {
    ...
    public CustomerResponse createCustomer(CustomerRequest customerRequest){
        ...
        Optional<Customer> customerByUserName = customerRepository.findByUserName(customerRequest.getUserName());
    ...
    public CustomerResponse getCustomerByUserName(String userName) {
        log.info("fetching records for the customer with userName {}", userName);
        Optional<Customer> existingCustomer = customerRepository.findByUserName(userName);
        if(!existingCustomer.isPresent()){
            throw new CustomerNotFoundException(new Throwable("Invalid userName"));
        }
        log.info("found customer with email {} and customerId {}", existingCustomer.get().getEmail(), userName);
        CustomerResponse response =  mapper.customerToCustomerResponse(existingCustomer.get());
        return response;
    }
